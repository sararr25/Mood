...
export interface AgentSuggestion { id: string; title: string; detail: string; kind: 'reference' | 'palette' | 'type' | 'decision' | 'principle'; status: 'proposed' | 'applied' | 'rejected'; mutation: SuggestionMutation; image?: string; undo?: SuggestionMutation }
...
export interface Project { id: string; title: string; brief: string; currentPhase: Phase; projectVersion: number; references: Reference[]; referenceOrder: string[]; referenceGroups: ReferenceGroup[]; directions: Direction[]; selectedDirectionId: string; designDecisions: DesignDecision[]; designSystem: DesignSystem; suggestions: AgentSuggestion[]; ignoredCritiqueIds: string[]; activity: ActivityEvent[] }
