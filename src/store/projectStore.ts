...
export const useProjectStore = create<ProjectStore>()(persist((set, get) => {
  ...
  return {
    ...
    applySuggestion: (suggestionId) => { const suggestion = getActiveProject(get())?.suggestions.find((item) => item.id === suggestionId); if (!suggestion || suggestion.status !== 'proposed') return { success: false, reason: 'Suggestion is no longer proposed.' }; mutateActive(`Approved and applied: ${suggestion.title}`, (project) => { const current = project.suggestions.find((item) => item.id === suggestionId); if (!current || current.status !== 'proposed') return project; const applied = applyMutation(project, current.mutation); return { ...applied.project, suggestions: applied.project.suggestions.map((item) => item.id === suggestionId ? { ...item, status: 'applied', undo: applied.undo } : item) } }, 'human'); return { success: true } },
    ...
    addSuggestion: (suggestion) => mutateActive(suggestion.origin === 'agent' ? 'Agent proposed a change' : suggestion.origin === 'critique' ? 'System surfaced a critique' : 'Guided example added a suggestion', (project) => ({ ...project, suggestions: [...project.suggestions, suggestion] }), suggestion.origin === 'agent' ? 'agent' : 'human'),
